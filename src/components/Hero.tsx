...
export function Hero() {
  return (
    <div id="top" className="relative overflow-hidden">
      ...
      {/* vivid, layered gradient wash — the "dynamic background" pass:
          more color, more motion than the previous single-tone orbs */}
      <div
        className="pointer-events-none absolute inset-x-0 top-0 -z-20 h-[780px] opacity-90"
        style={{
          background:
            "radial-gradient(900px 500px at 8% -8%, rgba(255,111,216,0.16), transparent 60%), radial-gradient(1000px 550px at 95% 0%, rgba(110,168,255,0.16), transparent 55%), radial-gradient(700px 420px at 45% 90%, rgba(167,139,250,0.14), transparent 60%)",
        }}
      />
      ...
      <div
        ...
        style={{ background: "radial-gradient(circle, rgba(255,111,216,0.22), transparent 70%)" }}
      />
      <div
      ...
      />
      <div
        className="orb animate-float-slower right-1/4 top-8 h-56 w-56 opacity-40"
        style={{ background: "radial-gradient(circle, rgba(110,168,255,0.2), transparent 70%)" }}
      />
      ...
      >
        <div>
          ...
          <motion.div variants={item} className="mt-10 flex flex-wrap items-center gap-4">
            <Link
              href="/console"
              className="btn-shine glow-accent rounded-full bg-accent px-6 py-3 text-sm font-semibold text-on-accent transition-transform hover:scale-[1.03]"
            >
              Launch Live Console →
            </Link>
            <a
      ...
      <WaveDivider className="pointer-events-none absolute -bottom-1 left-0 -z-10 h-[140px] w-full sm:h-[180px]" />
    </div>
  );
}
